...
import time
from lib import MQTTClient as mqtt
from time import sleep

#default MQTT broker
mqttHost = "192.168.1.37"
#default MQTT broker port
mqttPort = "1883"
#default MQTT topic prefix
mqttTopicPrefix = "motionEye"
#mqtt client
mqc = None
...
def connecthandler(mqc, userdata, rc):
    logging.debug("MQTT: Connected to MQTT broker with rc=%d" % (rc))
    mqc.subscribe(topic + "command/#", qos=0)

def disconnecthandler(mqc, userdata, rc):
    logging.debug("MQTT: Disconnected from MQTT broker with rc=%d" % (rc))
    time.sleep(5)
    mqc.reconnect()
...
def msghandler(mqc, userdata, msg):
    try:
        ...
        if mytopic.startswith("command/"):
            logging.debug(mytopic[8:], msg.payload)
    except Exception as e:
        logging.debug("MQTT: Error processing message %s: %s" % (type(e).__name__, e))
...
def createMQTTClient(RelayEventHandler):
    ...
    if RelayEventHandler.mqc:
        logging.debug("already have MQTT")
        return (True)
    else:
        mqc = mqtt.Client()
        ...
        if (username == main_config.get('@admin_username') and
                (signature == utils.compute_signature(self.request.method, self.request.uri,  # backwards compatibility
                         ...
                         signature == utils.compute_signature(self.request.method, self.request.uri,
                                                              self.request.body,
                                                              hashlib.sha1(
                                                                  main_config['@admin_password']).hexdigest()))):
            ...
            return 'normal'
                  ...
                  (signature == utils.compute_signature(self.request.method, self.request.uri,
                           ...
                           signature == utils.compute_signature(self.request.method, self.request.uri,
                                                                self.request.body,
                                                                hashlib.sha1(
                                                                    main_config['@normal_password']).hexdigest()))):
...
class PictureHandler(BaseHandler):
    ...
    def frame(self, camera_id):
        ...
        if utils.is_local_motion_camera(camera_config) or utils.is_simple_mjpeg_camera(
                camera_config) or self.get_argument('title', None) is not None:
            self.render('main.html',
                        frame=True,
                        camera_id=camera_id,
