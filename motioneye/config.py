...
def get_main(as_lines=False):
    ...
    main_config = _conf_to_dict(lines,
                                list_names=['thread'],
                                no_convert=['@admin_username', '@admin_password', '@normal_username',
                                            '@normal_password'])
...
def motion_camera_ui_to_dict(ui, old_config=None):
    ...
    print ("broker : %s",ui.get('mqttBroker'))

    data = {

        # MQTT
        '@mqttEnabled': ui['mqttEnabled'],
        '@mqttClientId': ui['mqttClientId'],
        '@mqttBroker': ui['mqttBroker'],
        '@mqttBrokerPort': ui['mqttBrokerPort'],
        '@mqttTopicPrefix': ui['mqttTopicPrefix'],
        '@mqttUser': ui['mqttUser'],
        '@mqttPassword': ui['mqttPassword'],
        '@mqttUseTLS': ui['mqttUseTLS'],
        '@mqttNoAuth': ui['mqttNoAuth'],
    ...
    if motionctl.needs_ffvb_quirks():
        if data['ffmpeg_video_codec'] in _EXPONENTIAL_QUALITY_CODECS:
            vbr = max(1, _MAX_FFMPEG_VARIABLE_BITRATE * (
            1 - math.log(max(1, q * _EXPONENTIAL_QUALITY_FACTOR), _EXPONENTIAL_QUALITY_FACTOR * 100)))
    ...
    if ui['mask']:
        if ui['mask_type'] == 'smart':
            ...
            data['mask_file'] = utils.build_editable_mask_file(old_config['@id'], ui['mask_lines'], data.get('width'),
                                                               data.get('height'))
    ...
    if ui['email_notifications_enabled']:
        ...
        on_event_start.append(
            "%(script)s '%(server)s' '%(port)s' '%(account)s' '%(password)s' '%(tls)s' '%(from)s' '%(to)s' 'motion_start' '%%t' '%%Y-%%m-%%dT%%H:%%M:%%S' '%(timespan)s'" % {
...
def motion_camera_dict_to_ui(data):
    ...
    print ("broker ui : -  : %s", data.get('@mqttBroker'))

    ui = {
        # MQTT
        'mqttEnabled': data.get('@mqttEnabled','false'),
        'mqttClientId': data.get('@mqttClientId'),
        'mqttBroker': data.get('@mqttBroker'),
        'mqttBrokerPort': data.get('@mqttBrokerPort'),
        'mqttTopicPrefix': data.get('@mqttTopicPrefix'),
        'mqttUser': data.get('@mqttUser'),
        'mqttPassword': data.get('@mqttPassword'),
        'mqttUseTLS': data.get('@mqttUseTLS','false'),
        'mqttNoAuth': data.get('@mqttNoAuth','false'),
...
def backup():
    ...
    if len(os.listdir(settings.CONF_PATH)) > 100:
        logging.debug(
            'config path "%s" appears to be a system-wide config directory, performing a selective backup' % settings.CONF_PATH)
        ...
        logging.debug(
            'config path "%s" appears to be a motion-specific config directory, performing a full backup' % settings.CONF_PATH)
